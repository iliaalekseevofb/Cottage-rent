...
const ImageSlider = () => {
  ...
  return (
    <div className='w-full h-[220px] esm:h-[300px] sm:h-[400px] lg:h-[500px] xl:h-[540px] relative overflow-hidden'>
      {sources.map((item, index) => {
        return (
          <div key={index}>
            ...
            <div className='absolute z-20 top-[16%] sm:top-[24%] lg:top-[20%] xl:top-[28%] left-[10%]'>
              <h1 className='text-white font-sans text-xl esm:text-2xl sm:text-3xl lg:text-4xl font-semibold'>{headers[slideIndex-1]}</h1>
              <h3 className='my-3 esm:my-5 lg:my-8 max-w-[260px] esm:max-w-[320px] sm:max-w-[480px] lg:max-w-[580px] text-white font-serif text-sm esm:text-md sm:text-xl'>{texts[slideIndex-1]}</h3>
              <button className='hidden esm:block px-6 md:px-8 py-3 md:py-4 text-white text-base esm:text-md lg:text-xl rounded-md bg-orange-400 hover:brightness-75 duration-200'>Подробнее</button>
            </div>
          </div>
        )
      })}
      <div className='absolute overflow-hidden hidden xl:block z-20 top-[10%] bottom-[10%] right-[10%] w-[360px] bg-white rounded-xl shadow-xl p-7'>
        <h1 className='text-2xl font-bold'>Закажите звонок</h1>
        <h3 className='text-lg font-serif mb-4 mt-4'>Оставьте заявку и мы перезвоним вам в течение 15 минут</h3>
        <input type='text' placeholder='* Имя:' className='w-full px-2 py-2 text-md rounded-md border border-neutral-400 outline-1 outline-green-800 mb-3' />
        <input type='text' placeholder='* Телефон:' className='w-full px-2 py-2 text-md rounded-md border border-neutral-400 outline-1 outline-green-800 ' />
        <p className='mt-4 mb-4'>Отправляя заявку, вы соглашаетесь с <Link className='text-green-600' to='/privacy-policy'>Политикой конфиденциальности</Link></p>
